...
const datas = ref<Array<SafetyPatrol>>([]);
...
const filter = reactive<Filter<SafetyPatrol>>({
  username: { op: "eq", value: undefined }
});

const formRef = ref<FormInstance>();
const initialValues: SafetyPatrol = {
  id: null,
  username: "",
  fullName: "",
  email: "",
...
const form = ref<SafetyPatrol>({ ...initialValues });
...
const rules = reactive<FormRules<typeof form>>({
  ...
  fullName: [
    {
      required: true,
      message: t("placeholder.inputText", { field: t("label.fullName") }),
      trigger: "blur"
    }
  ],
  email: [
    {
      required: true,
      message: t("placeholder.inputText", { field: t("label.email") }),
      trigger: "blur"
    }
  ]
});
 ...
 * @param row 数据
 */
function saveRow(row?: SafetyPatrol) {
 ...
 * @param row 数据
 */
async function enableRow(row: SafetyPatrol) {
  const id = row.id;
  if (!id) return;

  try {
    const res = await enableSafetyPatrol(id);
    if (res.data) {
      row.enabled = true;
    }
 ...
 * @param row 数据
 */
async function disableRow(row: SafetyPatrol) {
  const id = row.id;
  if (!id) return;
  ...
  await ElMessageBox.confirm(t("tips.disableWarning"), t("tips.confirm"), {
    ...
    try {
      const res = await disableSafetyPatrol(id);
      if (res.data) {
        row.enabled = false;
      }
...
async function onSubmit(formEl: FormInstance) {
  ...
  if (valid) {
    try {
      ...
      if (form.value.id) {
        await modifySafetyPatrol(form.value.id, form.value);
      } else {
        await createSafetyPatrol(form.value);
      }
      visible.value = false;
