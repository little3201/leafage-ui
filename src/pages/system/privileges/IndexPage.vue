          ...
          <q-card-section>
            ...
            <div v-if="!form.redirect" class="row q-gutter-xs">
              <q-chip
                ...
                :key="idx"
                :selected="form.actions?.includes(item.name)"
                :color="actionTypes[item.name]"
              >
                {{ $t(item.name) }}
              </q-chip>
            </div>
        ...
        <q-tr :props="props">
          ...
          <q-td v-for="col in props.cols" :key="col.name">
            ...
            <div v-else-if="col.name === 'name'">
              <q-icon
                :name="`sym_r_${pageIcons[props.row.name]}`"
                size="sm"
                class="q-pr-sm"
              />{{ $t(`page.${col.value}`) }}
            </div>
            ...
            >
              ...
              <template v-if="props.row.actions.length > 3">
                ...
                >
                  ...
                  <q-tooltip>
                    <q-chip
                      v-for="(item, index) in props.row.actions.slice(3)"
                      :key="index"
                      :label="$t(`action.${item}`)"
                      :color="actionTypes[item]"
                      text-color="white"
