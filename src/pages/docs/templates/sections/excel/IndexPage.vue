...
<template>
  <ExcelContent v-if="excelMode === 'DATA'" :section-id="sectionId" :read-only="readOnly" />
  <ExcelField v-else-if="excelMode === 'FIELD'" :section-id="sectionId" :read-only="readOnly" />
  <ExcelRender v-else-if="excelMode === 'RENDER'" :section-id="sectionId" :name="name" :read-only="readOnly" />
</template>