...
const datas = ref<Array<Scheduler>>([]);
...
const filter = reactive<Filter<Scheduler>>({
 ...
 * @param row 数据
 */
function saveRow(row?: Scheduler) {
 ...
 * @param row 数据
 */
async function enableRow(row: Scheduler) {
  const id = row.id;
  if (!id) return;

  try {
    const res = await enableScheduler(id);
    if (res.data) {
      row.enabled = true;
    }
 ...
 * @param row 数据
 */
async function disableRow(row: Scheduler) {
  const id = row.id;
  if (!id) return;
  ...
  await ElMessageBox.confirm(t("tips.disableWarning"), t("tips.confirm"), {
    ...
    try {
      const res = await disableScheduler(id);
      if (res.data) {
        row.enabled = false;
      }
...
async function onSubmit(formEl: FormInstance) {
  ...
  if (valid) {
    ...
    try {
      if (form.value.id) {
        await modifyScheduler(form.value.id, form.value);
      } else {
        await createScheduler(form.value);
      }
      visible.value = false;
