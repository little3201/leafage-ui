      ...
      <q-card>
        ...
        <q-card-section>
          <div class="row q-gutter-md">
            ...
            >
            <p
              ><strong>{{ $t("label.startTime") }}</strong>
              {{
                row.startTime
                  ? date.formatDate(row.startTime, "YYYY-MM-DD HH:mm")
                  : "-"
              }}
            </p>
            <p>
              ...
              <q-chip
                size="sm"
                :color="shceduleStatus[row.status || '']"
                text-color="white"
              >
                <q-icon
                  :name="`sym_r_${shceduleStatusIcon[row.status || '']}`"
          ...
          <div class="q-gutter-md">
            ...
            <p
              ><strong>{{ $t("label.nextExecuteTime") }}</strong>
              {{
                row.nextExecuteTime
                  ? date.formatDate(row.nextExecuteTime, "YYYY-MM-DD HH:mm")
                  : "-"
              }}
            </p>
            <p
              ><strong>{{ $t("label.record") }}</strong>
              {{ row.record }}
            </p>
          </div>
        </q-card-section>
    ...
    >
      ...
      <template v-slot:body-cell-startTime="props">
        <q-td :props="props">
          {{
            props.row.startTime
              ? date.formatDate(props.row.startTime, "YYYY-MM-DD HH:mm")
              : "-"
          }}
        </q-td>
      </template>
      <template v-slot:body-cell-status="props">
        <q-td :props="props">
          <q-chip :color="shceduleStatus[props.row.status]" text-color="white">
            <q-icon
              :name="`sym_r_${shceduleStatusIcon[props.row.status]}`"
              :class="[props.row.status === 'RUNNING' ? 'spin' : '', 'q-mr-xs']"
              size="xs"
            />
            {{ props.row.status }}
          </q-chip>
      ...
      <template v-slot:body-cell-nextExecuteTime="props">
        <q-td :props="props">
          {{
            props.row.nextExecuteTime
              ? date.formatDate(props.row.nextExecuteTime, "YYYY-MM-DD HH:mm")
              : "-"
          }}
        </q-td>
      </template>
