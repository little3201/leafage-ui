...
import { actionTypes, httpTypes } from "@/constants";
...
async function removeRow(id: number, httpMethod: string, url: string) {
  ...
  await ElMessageBox.confirm(
    t("tips.removeWarning", {
      module: t("page.accessLogs"),
      data: httpMethod + ' (url: "' + url + '")'
    }),
    t("tips.confirm"),
    {
  ...
  <ElDialog v-model="visible" :title="$t('action.details')" width="600">
    <ElDescriptions border>
      ...
      <ElDescriptionsItem :label="$t('label.statusCode')">
        ...
        <ElTag
          v-else-if="data.statusCode && data.statusCode >= 500"
          type="warning"
          round
        >
          {{ data.statusCode }}
      ...
      <ElDescriptionsItem :label="$t('label.ip')" :span="3">{{
        data.ip
      }}</ElDescriptionsItem>
